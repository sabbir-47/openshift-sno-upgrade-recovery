...
func launchBackupImages(client client.Client, Spoke string, BinaryImage string, BackupPath string) error {
...
func removePreviousPolicy(client client.Client, Spoke string, BinaryImage string, BackupPath string) error {
...
var backupImagesCmd = &cobra.Command{
	Use:   "backupImages",
	Short: "It will trigger the backup of the current images on an spoke. Needs to be run at hub cluster",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		err = removePreviousPolicy(client, Spoke, BinaryImage, BackupPath)
		if err != nil {
			return err
		}

		err = launchBackupImages(client, Spoke, BinaryImage, BackupPath)
		if err != nil {
			return err
		}
...
func init() {
	...
	rootCmd.AddCommand(backupImagesCmd)

	backupImagesCmd.Flags().StringP("KubeconfigPath", "k", "", "Path to kubeconfig file")
	backupImagesCmd.MarkFlagRequired("KubeconfigPath")

	backupImagesCmd.Flags().StringP("Spoke", "s", "", "Path to the spoke cluster")
	backupImagesCmd.MarkFlagRequired("Spoke")

	backupImagesCmd.Flags().StringP("BinaryImage", "l", "quay.io/yrobla/openshift-ai-image-backup", "Path to the binary for image backups")
	backupImagesCmd.Flags().StringP("BackupPath", "p", "/var/recovery", "Path where to store the backups")

	// bind to viper
	viper.BindPFlag("KubeconfigPath", backupImagesCmd.Flags().Lookup("KubeconfigPath"))
	viper.BindPFlag("Spoke", backupImagesCmd.Flags().Lookup("Spoke"))
	viper.BindPFlag("BinaryImage", backupImagesCmd.Flags().Lookup("BinaryImage"))
	viper.BindPFlag("BackupPath", backupImagesCmd.Flags().Lookup("BackupPath"))
}
